...
// Static Navigation Options including Deep Links
const STATIC_ACTIONS: SearchResult[] = [
    { id: 'home', title: 'Dashboard', subtitle: 'Go to Home', type: 'NAV', payload: { view: 'DASHBOARD' }, icon: LayoutDashboard },
    { id: 'metrics', title: 'Metrics', subtitle: 'Analytics & Reports', type: 'NAV', payload: { view: 'METRICS' }, icon: FileText },
    // Profiles Deep Links
    { id: 'prof_search', title: 'Search Profiles', subtitle: 'Candidate Search', type: 'NAV', payload: { view: 'PROFILES', subView: 'SEARCH' }, icon: Search },
    { id: 'prof_folders', title: 'Folders', subtitle: 'Manage Candidate Folders', type: 'NAV', payload: { view: 'PROFILES', subView: 'FOLDERS' }, icon: Folder },
    { id: 'prof_tags', title: 'Tags Management', subtitle: 'Manage Global Tags', type: 'NAV', payload: { view: 'PROFILES', subView: 'TAGS' }, icon: Tag },
    // Settings Deep Links
    { id: 'set_company', title: 'Company Settings', subtitle: 'Company Info', type: 'NAV', payload: { view: 'SETTINGS', settingsTab: 'COMPANY_INFO' }, icon: Building2 },
    { id: 'set_users', title: 'User Management', subtitle: 'Manage Team Members', type: 'NAV', payload: { view: 'SETTINGS', settingsTab: 'USERS' }, icon: Users },
    { id: 'set_roles', title: 'Roles & Permissions', subtitle: 'Access Control', type: 'NAV', payload: { view: 'SETTINGS', settingsTab: 'ROLES' }, icon: Shield },
    { id: 'set_layout', title: 'ReachOut Layouts', subtitle: 'Customize Interface', type: 'NAV', payload: { view: 'SETTINGS', settingsTab: 'REACHOUT_LAYOUTS' }, icon: Layout },
    // Account
    { id: 'my_account', title: 'My Account', subtitle: 'Personal Settings', type: 'NAV', payload: { view: 'MY_ACCOUNT' }, icon: User },
];
    ...
    GLOBAL_CAMPAIGNS.forEach(c => {
      if (c.name.toLowerCase().includes(lowerQuery) || c.jobID.includes(query)) {
        searchResults.push({
          ...
          type: 'CAMPAIGN',
          title: c.name,
          subtitle: `Campaign • ${c.jobID} • ${c.status}`,
          icon: Briefcase,
          payload: c
        });
      }
    ...
    MOCK_PROFILES.forEach(p => {
      if (p.name.toLowerCase().includes(lowerQuery) || p.title.toLowerCase().includes(lowerQuery)) {
        searchResults.push({
          ...
          type: 'CANDIDATE',
          title: p.name,
          subtitle: `Candidate • ${p.title} • ${p.location}`,
          icon: User,
          payload: p
        });
      }
