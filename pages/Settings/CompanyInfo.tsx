...
export const CompanyInfo = () => {
    ...
    const [isFranchised, setIsFranchised] = useState(false);
            ...
            try {
                ...
                if (data && data.companyProfile) {
                    ...
                    setFormData({
                        ...
                        companyAlias: profile.companyNameAlias?.[0] || '',
                        description: profile.companyDescription || '',
                        url: profile.companyURL || '',
                        department: settings.department || 'Human Resources',

                        // Location & Locale
                        continent: profile.continent || 'North America',
                        country: profile.country || 'United States',
                        countryCode: profile.countryCode || 'US',
                        operatingLocations: profile.companyLocations || [],
                        intlPhoneCode: profile.internationalPhoneCode || '+1',
                        currency: settings.defaultCurrency || 'USD',
                        language: profile.languages?.[0]?.language || 'English (US)',

                        // Time & Date
                        timeZone: profile.defaultTimeZone || '(GMT-05:00) Eastern Time',
                        dateFormat: profile.defaultDateFormat || 'MM/DD/YYYY',
                        timeFormat: profile.defaultTimeFormat === '24 hours' ? '24h' : '12h',

                        // Configurations
                        excludeAllDay: settings.calendarSettings?.excludeAllDay?.enabled ?? true,
                        doNotDisturb: settings.doNotDisturb || false,
                        profileActivityDuration: settings.profileActivityDuration || 90,
                        jobRecAccessLevel: settings.jobRecommendationsAccessLevel || 'Company'
                    });

                    if (profile.companyLogo) setLogoPreview(profile.companyLogo);
                    if (profile.companyMinifiedLogo) setShortLogoPreview(profile.companyMinifiedLogo);
                }
            } catch (error) {
                console.error("Failed to load company data:", error);
                addToast(t("Failed to load company information"), "error");
            }
        };
    ...
    const handleCancel = () => {
        ...
        setIsEditing(false);
        addToast(t("Changes discarded"), "info");
    };

    const handleSave = async () => {
        const saveAction = async () => {
            // Map flat form data back to nested company structure (partial update)
            const updatePayload = {
                companyProfile: {
                    companyName: formData.companyName,
                    companyNameAlias: [formData.companyAlias],
                    companyDescription: formData.description,
                    companyURL: formData.url.startsWith('http') ? formData.url : `https://${formData.url}`,
                    continent: formData.continent,
                    country: formData.country,
                    countryCode: formData.countryCode,
                    companyLocations: formData.operatingLocations,
                    internationalPhoneCode: formData.intlPhoneCode,
                    defaultTimeZone: formData.timeZone,
                    defaultDateFormat: formData.dateFormat,
                    defaultTimeFormat: formData.timeFormat === '24h' ? '24 hours' : '12 hours',
                    languages: [{ language: formData.language, primary: true }]
                },
                productSettings: {
                    department: formData.department,
                    defaultCurrency: formData.currency,
                    profileActivityDuration: formData.profileActivityDuration,
                    jobRecommendationsAccessLevel: formData.jobRecAccessLevel,
                    calendarSettings: {
                        excludeAllDay: { enabled: formData.excludeAllDay }
                    }
                }
            };

            await companyService.update(updatePayload);
            console.log("Saving Company Info:", updatePayload);
        };

        await addPromise(saveAction(), {
            loading: t('Saving company settings...'),
            success: t('Company information updated successfully.'),
            error: t('Failed to save settings.')
        });
    ...
    return (
        <SafeKeeper permissionPath={['System & Administration', 'settings', 'companyInfo']} mode="warn">
                    ...
                    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 dark:border-slate-700 pb-6">
                            <div>
                                ...
                                <p className="text-slate-500 dark:text-slate-400 mt-1">
                                    {t("Manage your organization's profile, branding, and global settings.")}
                                </p>
                            </div>
                        ...
                        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-xl flex gap-3">
                            ...
                            <div className="text-sm text-blue-800 dark:text-blue-300">
                                <p className="font-bold mb-1">{t("Company-Wide Settings")}</p>
                                <p className="text-xs opacity-90">{t("Changes made here will affect all users within this organization tenant. Ensure accuracy before saving.")}</p>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                            ...
                            <div className="xl:col-span-2 space-y-8">
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6 transition-opacity duration-300">
                                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-6 flex items-center gap-2">
                                        <Image size={16} className="text-slate-400" /> {t("Branding & Logos")}
                                    </h4>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                                        {/* Main Logo */}
                                        <div className="space-y-3">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Company Logo")}</label>
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div className="flex-1 text-xs text-slate-500 dark:text-slate-400">
                                                    <p>{t("Recommended size: 400x400px.")}</p>
                                                    <p className="mt-1">{t("JPG, PNG, or SVG allowed.")}</p>
                                                    {isEditing && (
                                        ...
                                        {/* Short Logo */}
                                        <div className="space-y-3">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Company Short Logo")}</label>
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div className="flex-1 text-xs text-slate-500 dark:text-slate-400">
                                                    <p>{t("Used for favicon and collapsed sidebar.")}</p>
                                                    <p className="mt-1">{t("Recommended size: 64x64px.")}</p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Company Name")}</label>
                                            <input
                                                type="text"
                                                name="companyName"
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Company Name Alias")}</label>
                                            <input
                                                type="text"
                                                name="companyAlias"
                                        ...
                                        <div className="space-y-1.5 md:col-span-2">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Company URL")}</label>
                                        ...
                                        <div className="space-y-1.5 md:col-span-2">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Company Description")}</label>
                                            <textarea
                                                name="description"
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-6 flex items-center gap-2">
                                        <Globe size={16} className="text-slate-400" /> {t("Geography & Location")}
                                    </h4>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Primary Continent")}</label>
                                            <select
                                                name="continent"
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Primary Country")}</label>
                                            <select
                                                name="country"
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Country Code")}</label>
                                            <input
                                                type="text"
                                                name="countryCode"
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Intl. Phone Code")}</label>
                                            <input
                                                type="text"
                                                name="intlPhoneCode"
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Operating Countries")}</label>
                                        <div className={`border border-slate-200 dark:border-slate-600 rounded-lg p-3 ${!isEditing ? 'bg-slate-100 dark:bg-slate-800' : 'bg-white dark:bg-slate-700'}`}>
                                            <div className="flex flex-wrap gap-2 mb-2">
                                                ...
                                                {formData.operatingLocations.length === 0 && <span className="text-sm text-slate-400 italic">{t("No locations selected")}</span>}
                                            </div>
                                        ...
                                        <p className="text-[10px] text-slate-400 mt-1">{t("Define all countries where your company operates for multi-locale features.")}</p>
                                    </div>
                                </div>

                                {/* Franchise Model Preview */}
                                {isFranchised && (
                                    <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/10 dark:to-purple-900/10 rounded-xl p-6 border border-indigo-100 dark:border-indigo-800 shadow-sm animate-in fade-in slide-in-from-top-4 duration-500 mt-8">
                                        <div className="flex items-center justify-between mb-4">
                                            <h4 className="text-sm font-bold text-indigo-800 dark:text-indigo-300 uppercase tracking-wider flex items-center gap-2">
                                                <Building2 size={16} /> {t("Franchise Model Active")}
                                            </h4>
                                            <span className="px-2 py-0.5 bg-indigo-600 text-white text-[10px] font-bold rounded uppercase">{t("Preview")}</span>
                                        </div>
                                        <p className="text-sm text-indigo-600 dark:text-indigo-400 mb-6 leading-relaxed">
                                            {t("This organization is configured as a Franchise. You have access to centralized reporting and branch management features across all linked locations.")}
                                        </p>

                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div className="p-4 bg-white/50 dark:bg-slate-900/50 rounded-lg border border-indigo-200/50 dark:border-indigo-800/50">
                                                <div className="text-2xl font-bold text-indigo-700 dark:text-indigo-300">12</div>
                                                <div className="text-xs text-indigo-500">{t("Active Branches")}</div>
                                            </div>
                                            <div className="p-4 bg-white/50 dark:bg-slate-900/50 rounded-lg border border-indigo-200/50 dark:border-indigo-800/50">
                                                <div className="text-2xl font-bold text-indigo-700 dark:text-indigo-300">450+</div>
                                                <div className="text-xs text-indigo-500">{t("Shared Talent Pool")}</div>
                                            </div>
                                        </div>

                                        <button className="w-full mt-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg transition-colors shadow-sm">
                                            {t("Manage Franchise Network")}
                                        </button>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-6 flex items-center gap-2">
                                        <Clock size={16} className="text-slate-400" /> {t("Regional & Format")}
                                    </h4>
                                    <div className="space-y-5">
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Default Language")}</label>
                                            <input
                                                type="text"
                                                name="language"
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Currency")}</label>
                                            <input
                                                type="text"
                                                name="currency"
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Time Zone")}</label>
                                            <select
                                                name="timeZone"
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Date Format")}</label>
                                                <select
                                                    name="dateFormat"
                                            ...
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Time Format")}</label>
                                                <select
                                                    name="timeFormat"
                                ...
                                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-6 flex items-center gap-2">
                                        <Settings size={16} className="text-slate-400" /> {t("System Configuration")}
                                    </h4>
                                    ...
                                    <div className="space-y-5">
                                        ...
                                        <div className={`flex items-center justify-between p-3 border border-slate-200 dark:border-slate-600 rounded-lg transition-colors ${isEditing ? 'hover:bg-slate-50 dark:hover:bg-slate-700/50' : 'opacity-70 bg-slate-50 dark:bg-slate-900/50'}`}>
                                            <div>
                                                <span className="text-sm font-medium text-slate-700 dark:text-slate-200 flex items-center gap-2">
                                                    <Moon size={16} className="text-indigo-500" /> {t("Do Not Disturb")}
                                                </span>
                                                <span className="text-xs text-slate-500 dark:text-slate-400 block mt-0.5">{t("Organization Default")}</span>
                                            </div>
                                        ...
                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Profile Activity Duration")}</label>
                                            <div className="relative">
                                                ...
                                                />
                                                <span className="absolute right-3 top-2.5 text-sm text-slate-500 dark:text-slate-400">{t("Days")}</span>
                                            </div>
                                            <p className="text-[10px] text-slate-400 mt-1">{t("Time before a profile is considered inactive.")}</p>
                                        </div>

                                        <div className="space-y-1.5">
                                            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">{t("Job Rec. Access Level")}</label>
                                            <select
                                                ...
                                                className="w-full px-3 py-2.5 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700 text-sm focus:ring-2 focus:ring-emerald-500 outline-none dark:text-slate-200 disabled:opacity-70 disabled:cursor-not-allowed disabled:bg-slate-100 dark:disabled:bg-slate-800"
                                            >
                                                <option>{t("Private (Owner Only)")}</option>
                                                <option>{t("Team Level")}</option>
                                                <option>{t("Company Level")}</option>
                                            </select>
                                        </div>
                                        ...
                                        <label className={`flex items-center justify-between p-3 border border-slate-200 dark:border-slate-600 rounded-lg transition-colors ${isEditing ? 'cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50' : 'opacity-70 bg-slate-50 dark:bg-slate-900/50'}`}>
                                            <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{t("Exclude All Day Events")}</span>
