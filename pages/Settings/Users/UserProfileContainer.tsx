...
export const UserProfileContainer = () => {
    ...
    const tabs = [
        ...
        { id: 'rolepermissions', label: 'Roles & Permissions', icon: Shield },
        { id: 'authsync', label: 'Auth & Sync', icon: Lock },
        { id: 'usernotifications', label: 'User Notifications', icon: Clock },
        { id: 'loginsessions', label: 'Login Sessions', icon: Activity },
    ...
    return (
        <div className="flex h-full flex-col bg-slate-50 dark:bg-slate-900 overflow-hidden">
            ...
            {/* Tab Navigation */}
            <div className="px-6 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex gap-6 shrink-0 overflow-x-auto no-scrollbar">
                {tabs.filter(t => ['basicdetails', 'settings', 'activities'].includes(t.id)).map(tab => {
                    const isActive = currentSection === tab.id;
                    return (
                        <button
                            key={tab.id}
                            onClick={() => navigate(`/settings/Users/userprofile/${tab.id}/${id}`)}
                            className={`py-4 text-sm font-medium border-b-2 transition-all whitespace-nowrap flex items-center gap-2 ${isActive ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                        >
                            <tab.icon size={16} />
                            {tab.label}
                        </button>
                        ...
                        case 'loginsessions':
                            return (
                                <div className="p-8 lg:p-12">
                                    <AccountPlaceholder
                                        title="Last Login Sessions"
                                        description={`Review recent login activity for ${userData?.firstName}.`}
                                        icon={Clock}
                                    />
                                </div>
                            );
...
const UserProfileSettings = ({ userData }: { userData: any }) => {
    ...
    const handleResetPassword = async () => {
        ...
        try {
            ...
            addToast('Password reset to Domain12!', 'success');
        } catch (e) {
    ...
    const handleToggleStatus = async () => {
        ...
        try {
            ...
            setStatus(newStatus);
            addToast(`User status updated to ${newStatus ? 'Active' : 'Inactive'}`, 'success');
        } catch (e) {
    ...
    return (
        <div className="p-8 lg:p-12 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="max-w-3xl mx-auto space-y-8">
                <div>
                    <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2">User Settings</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">Manage account status and security credentials.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-slate-500">Active or Inactive (Verified)</p>
                        </div>
                        <button
                ...
                <div className="p-6 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-xs text-slate-500">Resets password to default Domain12! format</p>
                        </div>
                        <button
        ...
        <div className="p-8 lg:p-12 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="max-w-3xl mx-auto">
                <AccountPlaceholder
                    ...
                    description={`View activity logs for user ID ${userId}. This feature is currently under development.`}
                    icon={Activity}
                />
            </div>
        </div>
