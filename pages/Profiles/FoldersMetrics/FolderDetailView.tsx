...
export const FolderDetailView = ({ folder, onBack }: { folder: any, onBack: () => void }) => {
   ...
   const handleAction = (type: string) => {
      ...
      if (type === 'TAG' || type === 'REFERRAL') {
         addToast(t("This feature will be available in the next update."), "info");
         return;
      }
      // @ts-ignore
      setActionType(type);
   ...
   const executeAction = () => {
      addToast(t("Action completed successfully!"), "success");
      setActionType(null);
      setSelectedIds(new Set());
   };
   ...
   return (
      <div className="h-full flex flex-col bg-white dark:bg-slate-900">
         ...
         <AccessControlModal
            ...
            currentSettings={folder.access} // Mock data might not have access, will default to private
            onSave={(settings) => {
               console.log("Saved folder sharing settings:", settings);
               addToast(t("Folder sharing settings updated"), "success");
            }}
         ...
         {/* MODALS */}
         {/* MODALS */}
         <ActionModal isOpen={actionType === 'ATTACH_CAMPAIGN'} onClose={() => setActionType(null)} title={t("Attach to Campaign")}>
            <div className="space-y-4">
               <p className="text-sm text-slate-600 dark:text-slate-300">
                  {t("Attaching")} <span className="font-bold text-emerald-600 dark:text-emerald-400">{selectedIds.size}</span> {t("profiles to")}:
               </p>
               ...
               <div className="pt-4 flex justify-end gap-3">
                  <button onClick={() => setActionType(null)} className="px-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 dark:text-slate-300">{t("Cancel")}</button>
                  <button onClick={executeAction} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold shadow-sm">{t("Attach")}</button>
               </div>
            </div>
         </ActionModal>

         <ActionModal isOpen={actionType === 'LINK'} onClose={() => setActionType(null)} title={t("Link to Folder")}>
            <div className="space-y-4">
               <p className="text-sm text-slate-600 dark:text-slate-300">
                  {t("Link")} <span className="font-bold text-emerald-600 dark:text-emerald-400">{selectedIds.size}</span> {t("profiles to")}:
               </p>
               <div className="space-y-2">
               ...
               <div className="pt-4 flex justify-end gap-3">
                  <button onClick={() => setActionType(null)} className="px-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 dark:text-slate-300">{t("Cancel")}</button>
                  <button onClick={executeAction} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-bold shadow-sm">{t("Link Profiles")}</button>
               </div>
            </div>
         </ActionModal>

         <ActionModal isOpen={actionType === 'COMM'} onClose={() => setActionType(null)} title={t("Send Communication")}>
            <div className="space-y-4">
               ...
               <p className="text-sm text-slate-600 dark:text-slate-300">
                  {t("Sending to")} <span className="font-bold text-emerald-600 dark:text-emerald-400">{selectedIds.size}</span> {t("recipients")}.
               </p>

               <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">
                     {activeCommTab === 'EMAIL' ? t('Subject') : t('Message Body')}
                  </label>
                  {activeCommTab === 'EMAIL' && (
                     <input type="text" placeholder={t("Enter email subject...")} className="w-full p-2.5 bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-lg text-sm outline-none focus:border-blue-500 dark:text-slate-200 mb-2" />
                  )}
                  <textarea
                     ...
                     placeholder={activeCommTab === 'EMAIL' ? t("Compose your email...") : t("Type your SMS message...")}
                  ></textarea>
               </div>
         ...
         <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white dark:bg-slate-900 sticky top-0 z-20">
            <div className="flex items-center gap-3">
               ...
               <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2 mt-0.5">
                     <span className="bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-slate-700">{folder.type}</span>
                     <span>•</span>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
               ...
               <button
                  ...
                  className="flex-1 md:flex-none px-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex items-center justify-center gap-2"
               >
                  <Briefcase size={16} /> {t("Attach to Campaign")}
               </button>
               <button className="flex-1 md:flex-none px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-bold shadow-sm transition-colors flex items-center justify-center gap-2">
                  <UserPlus size={16} /> {t("Add Profile")}
               </button>
            </div>
         </div>
         ...
         <div className="px-6 py-3 bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700 flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 w-full group">
               ...
               <input
                  type="text"
                  placeholder={t("Search profiles in this folder...")}
         ...
         {selectedIds.size > 0 && (
            <div className="px-6 py-2 bg-indigo-50 dark:bg-indigo-900/20 border-b border-indigo-100 dark:border-indigo-800 flex flex-col sm:flex-row justify-between items-center gap-3 animate-in slide-in-from-top-1">
               <div className="flex items-center gap-2 text-sm text-indigo-900 dark:text-indigo-200">
                  ...
                  <button onClick={() => setSelectedIds(new Set())} className="text-xs text-indigo-600 dark:text-indigo-400 hover:underline ml-2">{t("Clear")}</button>
               </div>
               <div className="flex items-center gap-2 overflow-x-auto w-full sm:w-auto">
                  <button onClick={() => handleAction('ATTACH_CAMPAIGN')} className="px-3 py-1.5 bg-white dark:bg-slate-800 border border-indigo-200 dark:border-indigo-800 rounded-lg text-xs font-bold text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 flex items-center gap-1 whitespace-nowrap transition-colors">
                     <Briefcase size={14} /> {t("Attach to Campaign")}
                  </button>
