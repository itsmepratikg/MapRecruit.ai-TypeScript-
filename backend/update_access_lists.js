...
const run = async () => {
    try {
        ...
        // 2. Prepare IDs
        // User expects ObjectIds
        const objectIds = clientIDs.map(id => new mongoose.Types.ObjectId(id));

        // Company (in this system) seems to use Strings in its clients array based on previous debugs
        // We will store Strings for Company to maintain consistency with existing data patterns there
        const stringIds = clientIDs;

        // 3. Update User
        user.clients = objectIds;
        await user.save();
        console.log('✅ User clients updated successfully.');

        // 4. Update Company
        // We use string lookup for _id as established in previous steps
        const result = await mongoose.connection.collection('companiesDB').updateOne(
            { _id: user.companyID.toString() },
            { $set: { clients: stringIds } }
        );

        if (result.matchedCount > 0) {
            console.log('✅ Company clients updated successfully.');
        } else {
            console.error('❌ Company not found or not updated.');
        }
